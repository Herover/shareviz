...
export const db = (function createDB() {
  ...
  return {
    ...
    duplicateSpec: (sourceIndex: number, insertAt: number) =>
      doc.submitOp(["specs", insertAt, { i: doc.data.specs[sourceIndex] }]),
    setConfigTitle: (configIndex: number, title: string) =>
      doc.submitOp(["specs", configIndex, "title", { r: 0, i: title }]),
    channelScope: (configIndex: number, channelKey: string) => {
      const scoped = createScope<Config.Channel>(db, [
        "specs",
        configIndex,
        "channels",
        channelKey,
